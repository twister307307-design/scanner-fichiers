...
SUSPICIOUS_NAME_PATTERNS = [
    "crack", "keygen", "trojan", "malware",
    "ransomware", "rootkit", "backdoor", "stealer",
    "rat_", "_rat", "miner", "botnet",
            ...
            if len(data) > 0:
                ...
                if entropy > 7.5:
                    safe_high_entropy = {
                        ...
                        ".igpi",
                        # Fichiers de jeux / launchers (Riot, Steam, Epic...) - donnees binaires
                        ".client", ".vpk", ".pak", ".pak2", ".vdf", ".manifest",
                        ".asset", ".assets", ".bundle", ".unity3d", ".uasset",
                        ".pck", ".rpf", ".forge", ".big", ".wad",
                    }
...
class ScannerApp:
        ...
        def _dl():
            try:
                ...
                if is_frozen:
                    ...
                    win.after(0, lambda: self._restart_after_update(win, lbl_status, prog, updater, current, is_frozen))

                else:
                    # .pyw : remplacement direct + relance simple
                    tmp = current + ".tmp"
    ...
    def _restart_after_update(self, win, lbl_status, prog, updater, current, is_frozen):
        import sys, subprocess
        prog.stop()
        if is_frozen:
            lbl_status.config(text="Mise a jour telechargee ! Recompilation en cours...", fg=self.GREEN)
        else:
...
"""
    ...
    def _clean_tab(self, notebook):
        ...
        # En-tete de section permanent (sous la barre info)
        tk.Label(frame, text="═══ FICHIERS SAINS ═══",
                 font=("Consolas", 8, "bold"), fg=self.GREEN, bg=self.BG2,
    ...
    def _log_tab(self, notebook, title, return_frame=False, section_title=None):
        frame = tk.Frame(notebook, bg=self.BG)
        notebook.add(frame, text=title)
        if section_title:
            tk.Label(frame, text=f"═══ {section_title} ═══",
    ...
    def _log_tab_with_action(self, notebook, title, btn_text, btn_cmd, btn_color, section_title=None):
        frame = tk.Frame(notebook, bg=self.BG)
        notebook.add(frame, text=title)
        if section_title:
            tk.Label(frame, text=f"═══ {section_title} ═══",
    ...
    def _scan_persistence(self, auto=False):
        """Detecte les programmes lances au demarrage de Windows (persistance).
        auto=True : analyse silencieuse au lancement (ne bascule pas l'onglet)."""
        if platform.system() != "Windows":
            if not auto:
        ...
        def _worker():
            ...
            def analyze_entry(name, command, source):
                ...
                if hard_reasons:
                    ...
                    extra = f" [{location_note}]" if location_note else ""
                    block = (f"\n  ⚠ {name}  [{source}]\n"
                             f"     {command}\n"
                    ...
                    sig_note = " | non signé" if sig_status == "unsigned" else ""
                    block = (f"\n  • {name}  [{source}]\n"
                             f"     {command}\n"
            ...
            if total == 0:
                self.root.after(0, lambda: self._log(
                    self.log_persist,
                    "\n\n  ─────────────────────────────\n"
                    "  Aucun élément de démarrage trouvé sur ce système.\n"
                    "  (le disque sélectionné ne contient aucun programme\n"
                    "   configuré pour se lancer au démarrage de Windows)",
                    "dim"))
            else:
                self.root.after(0, lambda: self._log(
                    self.log_persist,
            ...
            self.root.after(0, lambda: [
                ...
                self._set_status(
                    "Aucun élément de démarrage" if total == 0
